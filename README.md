...
```
The `--fuzzy` flag enables **Levenshtein-tolerant expansion**. If the LLM predicts `token_type_embeddings` but the corpus has `token_type_ids`, Skanda will still find it.
...
You can give your LLM access to Skanda by providing this tool definition:
